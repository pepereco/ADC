...
class lane_detection(object):
    def __init__(self):
      ...
      self.cmdVelocityDesvPub = rospy.Publisher("/control/front_left_desviation", Float64, queue_size=10)
      self.no_linePub= rospy.Publisher("/vision/no_line", String, queue_size=10)
      self.line_type_pub = rospy.Publisher("/vision/left_line_type", String, queue_size=10)
      ...
      self.corners = np.float32([[1.3*640/5, 3.5*480/5], [1.8*640/5, 3*480/5],[2.6*640/5, 3*480/5],[2.3*640/5, 3.5*480/5]]) #Gazebo Conde track
     ...
     while True:
         ...
         if self.imgRcvd:
             ...
             self.vertices = np.array( [[
                        [2.6*width/5, 3*height/5],
                        [1.8*width/5, 3*height/5],
                        [1.3*width/5, 3.5*height/5],
             ...
             cv2.imshow('left',self.maskedImage)
             cv2.waitKey(0)
             ...
             if fitx.shape[0] >1:
                 ...
                 radius = 5
                 car_x_center=640/2.
                 desv_center_sp=car_x_center - self.intersectionPoint
                 print("left: ", desv_center_sp)
                 self.cmdVelocityDesvPub.publish(desv_center_sp)
...
def main(args):
  ...
  rospy.init_node('real_left_line_detection', anonymous=True)
