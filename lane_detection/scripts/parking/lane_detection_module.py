...
def fit_polynomials(image, binary_warped, debug=False):
    ...
    histogram = np.sum(binary_warped, axis=1)
    ...
    window_length = np.int(binary_warped.shape[1]/nwindows)
    ...
    for window in range(nwindows):
        ...
        win_x_low = binary_warped.shape[1] - (window+1)*window_length
        win_x_high = binary_warped.shape[1] - window*window_length
