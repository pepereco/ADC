...
class parking_detection(object):
    ...
    def cb_enable(self, msg):
        ...
        if msg.data =="right_batery_detect" or msg.data =="left_detect":
            print ("park right deactiveted")
            self.parking_enable=0

    def callback_right_sonar(self, msg):
        self.sonar_right=msg.range
        self.sonar_right_avg = ld.movingAverage(self.sonar_right_avg, self.sonar_right,  N=10)
    ...
    def state_machine_parking(self):
        ...
        if self.state_parking == "detected":
            ...
            print ("sonar", self.sonar_right_avg)
            print ("encoders diff : ", self.encoders_diff_lines)
            #if self.sonar_right_avg > 0.5 and 50<self.encoders_diff_lines < 90:
            if 0.7 > self.sonar_right_avg > 0.5 and 50<self.encoders_diff_lines < 90:
                self.pub_detect.publish("right_detect")
                print ("park right detected")
            ...
            if (self.right_encoder-self.right_encoder_start < 87):
            ...
            if (self.right_encoder-self.right_encoder_start)> 48:
                self.counter+=1
                if self.counter>2:
                    print ("stop")
                    self.pub_park.publish("stop")
            ...
            if ((self.right_encoder-self.right_encoder_start) < 33):
                #self.counter=0
            ...
            #if (abs(self.yaw - self.starting_yaw)> 0.2 and (self.right_encoder -self.right_encoder_start > 25)):
            ...
            if (self.right_encoder -self.right_encoder_start > 20):
                ...
                print ("left_back: ", self.intersectionPoint)
                self.pub_park.publish("left_back")
        elif self.state_parking=="park_4":
            #if abs(self.yaw- self.starting_yaw)> 0.2 or (self.right_encoder -self.right_encoder_start > 20) :
            if (self.right_encoder -self.right_encoder_start > 15) :
                self.counter+=1
                if self.counter>5:
                    print ("stop")
                    self.pub_park.publish("stop")
            ...
            #if abs(self.yaw- self.starting_yaw)> 0.2 or (self.right_encoder -self.right_encoder_start > 20) :
            if (self.right_encoder -self.right_encoder_start > 12) :
                self.counter+=1
                if self.counter>5:
                    print ("stop")
                    self.pub_park.publish("stop")
    ...
    def state_machine_parking_out(self):
        ...
        if self.state_parking_out=="out" and self.state_parking=="parked":
            ...
            #if abs(self.yaw- self.starting_yaw)> 0.2 or (self.right_encoder -self.right_encoder_start > 20) :
            #if (self.right_encoder -self.right_encoder_start > 30) :
            if (self.right_encoder -self.right_encoder_start > 33) :
                self.counter+=1
                if self.counter>5:
                    print ("stop")
