...
class parking_detection(object):
    def __init__(self):
      ...
      self.corners = np.float32([[0*640/5, 3.5*480/5], [0.65*640/5, 3*480/5],[2.4*640/5, 3*480/5],[2.3*640/5, 3.5*480/5]]) #Gazebo Conde track
      ...
      self.sonar_left_avg=1.5
      self.detected_next_line=False
      ...
      self.sub_left_sonar=rospy.Subscriber('/adc_car/sensors/range/side_left_sonar', Range, self.callback_left_sonar)
    ...
    def cb_enable(self, msg):
        ...
        if msg.data =="right_batery_detect" or msg.data =="right_detect":
            print ("park left deactiveted")
            self.parking_enable=0


    def callback_left_sonar(self, msg):
        self.sonar_left=msg.range
        self.sonar_left_avg = ld.movingAverage(self.sonar_left_avg, self.sonar_left,  N=10)
        #print ("sonar:", self.sonar_left)
        #print ("sonar avg:", self.sonar_left_avg)
    ...
    def state_machine_parking(self):
        ...
        if self.state_parking == "detected":
            ...
            print ("sonar", self.sonar_left_avg)
            print ("encoders diff : ", self.encoders_diff_lines)
            #if self.sonar_left_avg > 1.5 and 50<self.encoders_diff_lines < 100:
            if self.sonar_left_avg > 0.5 and 50<self.encoders_diff_lines < 100:
                self.pub_detect.publish("left_detect")
                print ("park left detected")
            ...
            if (self.left_encoder-self.left_encoder_start < 85) and self.detected_next_line==False:
                print ("encoders: ", self.left_encoder-self.left_encoder_start)
                print ("intersection: ", self.intersectionPoint)
            else:
                print ("detected Next line", self.detected_next_line)
                self.left_encoder_start = self.left_encoder
                if self.detected_next_line==True:
                    while (self.left_encoder-self.left_encoder_start) <= 5:
                        self.pub_park.publish("front")
                self.detected_next_line=False
            ...
            print ("encoders diff: ", self.left_encoder-self.left_encoder_start)
            #print ("back cam: ", self.back_intersection_point)
            #if ((abs(self.yaw - self.starting_yaw ) > 0.5) and (self.intersectionPoint>570) and (self.left_encoder-self.left_encoder_start)> 48) :
            #if (self.intersectionPoint>580) and (abs(self.yaw - self.starting_yaw ) > 0.7) :
            if (self.left_encoder-self.left_encoder_start)> 45:
                self.counter+=1
                if self.counter>2:
                    print ("stop")
                    self.pub_park.publish("stop")
            ...
            if not(1400 < self.back_intersection_point < 1750) and ((self.left_encoder-self.left_encoder_start) < 33):
                #self.counter=0
                ...
                if time.time() -self.time_start > 5:
                    ...
                    if self.counter>4:
                        ...
                        self.starting_yaw=self.yaw
                        self.left_encoder_start= self.left_encoder
                        self.state_parking= "park_3"
                else:
                    print ("stoped by time")

        elif self.state_parking=="park_3":
            print ("park_3: ",abs(self.yaw - self.starting_yaw))
            #if abs(self.yaw - self.starting_yaw)> 0.2 or 100<self.intersectionPoint<200:
            #if (abs(self.yaw - self.starting_yaw)> 0.2 and (self.left_encoder -self.left_encoder_start > 25)):
            #if (self.left_encoder -self.left_encoder_start > 20):
            if (self.left_encoder -self.left_encoder_start > 20):
                self.counter+=1
                if self.counter>5:
                    print ("stop")
                    ...
                    self.starting_yaw=self.yaw
                    self.left_encoder_start= self.left_encoder
                    self.state_parking="park_4"
            else:
                print ("right_back: ", self.intersectionPoint)
                self.pub_park.publish("right_back")
        elif self.state_parking=="park_4":
            #if abs(self.yaw- self.starting_yaw)> 0.2 or (self.left_encoder -self.left_encoder_start > 20) :
            if (self.left_encoder -self.left_encoder_start > 15) :
                self.counter+=1
                if self.counter>5:
                    print ("stop")
                    self.pub_park.publish("stop")
                    ...
                    while time.time() -time_before < 1:
                        pass
                    self.left_encoder_start= self.left_encoder
                    self.state_parking="park_5"
            else:

                #print ("right_front: ", abs(abs(self.yaw)- abs(self.first_yaw )))
                self.pub_park.publish("left_front")
        elif self.state_parking=="park_5":
            #if abs(self.yaw- self.starting_yaw)> 0.2 or (self.left_encoder -self.left_encoder_start > 20) :
            if (self.left_encoder -self.left_encoder_start > 20) :
                self.counter+=1
                if self.counter>5:
                    ...
                    while time.time() -time_before < 1:
                        pass
                    self.left_encoder_start= self.left_encoder
                    self.state_parking="park_6"
            else:
                #print ("right_front: ", abs(abs(self.yaw)- abs(self.first_yaw )))
                self.pub_park.publish("right_back")
        elif self.state_parking=="park_6":
            #if abs(self.yaw- self.starting_yaw)> 0.2 or (self.left_encoder -self.left_encoder_start > 20) :
            if (self.left_encoder -self.left_encoder_start > 13) :
                self.counter+=1
                if self.counter>5:
                    print ("stop")
                    self.pub_park.publish("stop")
    ...
    def state_machine_parking_out(self):
        ...
        if self.state_parking_out=="out" and self.state_parking=="parked":
            ...
            #if abs(self.yaw- self.starting_yaw)> 0.2 or (self.left_encoder -self.left_encoder_start > 20) :
            #if (self.left_encoder -self.left_encoder_start > 30) :
            if (self.left_encoder -self.left_encoder_start > 33) :
                self.counter+=1
                if self.counter>5:
                    print ("stop")
                    self.pub_park.publish("stop")
                    ...
                    while time.time() -time_before < 1:
                        pass
                    self.left_encoder_start= self.left_encoder
                    self.state_parking_out="out_3"
            else:
                #print ("right_front: ", abs(abs(self.yaw)- abs(self.first_yaw )))
                self.pub_park.publish("right_front")
        elif self.state_parking_out == "out_3" :
            ...
            if self.control_alignment != "center":
                ...
                if self.counter_center==4:
                    self.pub_park.publish("stop")
                    time_before = time.time()
                    while time.time() -time_before < 0.5:
                        pass
                    time_before = time.time()
     ...
     while True:
         ...
         if self.imgRcvd:
             ...
             self.vertices = np.array( [[
                        [2.4*width/5, 3*height/5],
                        [0.65*width/5, 3*height/5],
                        [0*width/5, 3.5*height/5],
                        [2.3*width/5, 3.5*height/5]
                    ]], dtype=np.int32 )
             """
             ...
             """cv2.imshow('wraped left',self.warpedImage)
             cv2.waitKey(0)
             ...
             if (fitx.shape[0] >1 and ((np.max(fitx)- np.min(fitx))> 200)):
                 ...
                 """if self.parking_enable==1 and self.state_parking == "detect":
                     ...
                     if self.counter_detection > 100:
                         self.counter_detection=0
                         print ("detected")
                         self.encoders_diff_lines = self.left_encoder-self.encoders_past_detected_line
                         self.encoders_past_detected_line=self.left_encoder
                         self.state_parking= "detected"
                         self.state_machine_parking()
                 self.outputImage = self.processedImage"""

             if self.parking_enable==1 and (self.state_parking == "detect" or self.state_parking== "avance"):
                 #print ("detecting")
                 print ("counter if : ", self.counter_detection)
                 if self.counter_detection >=4:
                     self.counter_detection=0
                     print ("detected")
                     self.encoders_diff_lines = self.left_encoder-self.encoders_past_detected_line
                     if self.encoders_diff_lines > 70:
                         print ("im in more than 70")
                         self.encoders_past_detected_line=self.left_encoder
                         if self.state_parking == "detect":
                             self.state_parking= "detected"
                         elif self.state_parking=="avance":
                             print ("im in avance detect")
                             self.detected_next_line = True
                         self.state_machine_parking()
             self.outputImage = self.processedImage
